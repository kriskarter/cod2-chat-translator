...
**v1.17.1:** теперь в F9 работает Ctrl+V — можно вставлять скопированный текст прямо во время игры. В главном окне также явно показывается состояние входящего оверлея F8: ВКЛЮЧЕН, СКРЫТ или ВЫКЛЮЧЕН.

**v1.17.2:** входящий оверлей Windows переведён на нативную поканальную прозрачность. Устранена редкая чёрная вспышка при появлении новых сообщений. Количество видимых сообщений теперь можно выбрать от 4 до 8; по умолчанию — 4.

**v1.17.3:** исправлен ввод через F9 при активной украинской раскладке Windows. Исправлено определение активной раскладки Windows; украинская, русская и английская раскладки проверены непосредственно в игре.

**v1.17.4:** перевод стал быстрее и устойчивее к временным сбоям онлайн-сервисов. Входящий чат и F9 теперь используют короткие ограниченные сетевые запросы. Если основной переводчик тормозит, возвращает ошибку или просто отдаёт исходный текст без перевода, программа автоматически переключается на резервный сервис. Для входящего fallback исходный язык определяется локально. API-ключ пользователю не нужен.
...
> Для перевода нужен интернет. В онлайн-сервис уходит только текст распознанного сообщения чата. Начиная с v1.17.4, если основной переводчик отвечает слишком долго или временно недоступен, программа может автоматически использовать резервный онлайн-сервис. Полный `console_mp.log` никуда не отправляется.
...
Переводчик фильтрует файл локально. В активный сервис перевода уходит только текст уже распознанного сообщения чата, а не весь лог целиком. Основной путь использует публичный мобильный endpoint Google Translate; если в v1.17.4 требуется fallback, тот же отфильтрованный текст сообщения может быть отправлен в MyMemory. Параметры сервера и остальной вывод `console_mp.log` не отправляются.
